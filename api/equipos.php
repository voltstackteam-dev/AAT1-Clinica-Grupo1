...
ejecutarApi(function () use ($conexion, $metodo): void {
    ...
    if ($metodo === 'GET') {
        ...
        if (isset($_GET['id']) && !$data) {
            responderError('Equipo no encontrado.', 404);
        }
        responder([
            'success' => true,
    ...
    if ($sala && !existe($conexion, 'salas', 'id_sala', $sala)) {
        responderError('La sala no existe.');
    }
    $p = [
        ':nombre' => trim($d['nombre_equipo']),
        ':sala' => $sala,
        ':cantidad' => (int) $d['cantidad']
    ];

    if ($p[':cantidad'] < 1) {
        responderError('La cantidad debe ser mayor que cero.');
    }

    if ($metodo === 'POST') {
        $s = $conexion->prepare(
            'INSERT INTO equipos(nombre_equipo, id_sala, cantidad)
                VALUES (:nombre, :sala, :cantidad)'
        );
        $s->execute($p);
        responder([
            'success' => true,
            'id_equipo' => (int) $conexion->lastInsertId()
        ], 201);
    }
