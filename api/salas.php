...
ejecutarApi(function () use ($conexion, $metodo): void {
    ...
    if ($metodo === 'GET') {
        ...
        if (isset($_GET['id']) && !$data) {
            responderError('Sala no encontrada.', 404);
        }
        responder([
            'success' => true,
    ...
    if (
        $especialidad
        && !existe($conexion, 'especialidades', 'id_especialidad', $especialidad)
    ) {
        responderError('La especialidad no existe.');
    }

    if ($metodo === 'POST') {
        $stmt = $conexion->prepare(
            'INSERT INTO salas(nombre_sala, ubicacion, id_especialidad)
                VALUES (:nombre, :ubicacion, :especialidad)'
        );
        $stmt->execute([
            ':nombre' => trim($datos['nombre_sala']),
            ':ubicacion' => $datos['ubicacion'] ?? null,
            ':especialidad' => $especialidad
        ]);
        responder([
            'success' => true,
            'id_sala' => (int) $conexion->lastInsertId()
        ], 201);
    }
