...
ejecutarApi(function () use ($conexion, $metodo): void {
    ...
    if ($metodo === 'GET') {
        ...
        if (!$datos) {
            responderError('Paciente no encontrado.', 404);
        }
        responder([
            'success' => true,
    ...
    if ($metodo === 'POST') {
        ...
        try {
            if (!empty($datos['id_usuario'])) {
                ...
                if (!existe($conexion, 'usuarios', 'id_usuario', $idUsuario)) {
                    responderError('El usuario no existe.');
                }
            } else {
                requerirCampos($datos, ['email', 'contrasenia']);

                if (!filter_var($datos['email'], FILTER_VALIDATE_EMAIL)) {
                    responderError('Email inválido.');
                }
                $crearUsuario = $conexion->prepare(
                    'INSERT INTO usuarios(email, contrasenia, id_rol, activo)
                        VALUES (:email, :clave, 3, 1)'
                );
                $crearUsuario->execute([
                    ':email' => trim($datos['email']),
                    ':clave' => password_hash($datos['contrasenia'], PASSWORD_DEFAULT)
                ]);
            ...
            $crearPaciente = $conexion->prepare
                (
                    'INSERT INTO pacientes(id_usuario, nombre, apellido, fecha_nacimiento, telefono, direccion)
            ...
            responder([
                'success' => true,
                'id_paciente' => (int) $conexion->lastInsertId(),
    ...
    if ($metodo === 'PUT') {
        ...
        if ((int) $usuario->id_rol === 3) {
            if (perfilPaciente($conexion, (int) $usuario->id_usuario) !== $idPaciente) {
                responderError('Solo puede editar su propio perfil.', 403);
            }
