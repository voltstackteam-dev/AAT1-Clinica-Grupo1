...
function validarHorario(array $datos): array
{
    ...
    if ($inicio < '08:00:00' || $fin > '17:00:00' || $inicio >= $fin) {
        responderError('El horario debe estar entre 08:00 y 17:00 y tener una hora de inicio menor a la final.');
    }
...
ejecutarApi(function () use ($conexion, $metodo): void {
    ...
    if ($metodo === 'GET') {
        ...
        $consulta = $conexion->prepare(
            $sql . ' ORDER BY FIELD(d.dia_semana,"LUNES","MARTES","MIERCOLES","JUEVES","VIERNES"),d.hora_inicio'
        );
        $consulta->execute($parametros);
        $datos = isset($_GET['id']) ? $consulta->fetch() : $consulta->fetchAll();

        if (isset($_GET['id']) && !$datos) {
            responderError('Disponibilidad no encontrada.', 404);
        }
        responder([
            'success' => true,
    ...
    if (!existe($conexion, 'medicos', 'id_medico', $idMedico)) {
        responderError('El médico no existe.');
    }
    ...
    if ($conflicto->fetchColumn()) {
        responderError('Ese horario se cruza con una disponibilidad existente.', 409);
    }

    if ($metodo === 'POST') {
        $consulta = $conexion->prepare(
            'INSERT INTO disponibilidades(id_medico, dia_semana, hora_inicio, hora_fin)
                VALUES (:medico, :dia, :inicio, :fin)'
        );
        $consulta->execute([
            ':medico' => $idMedico,
            ':dia' => $dia,
        ...
        responder([
            'success' => true,
            'id_disponibilidad' => (int) $conexion->lastInsertId()
        ], 201);
    }
