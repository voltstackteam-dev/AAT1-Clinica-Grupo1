...
function validarDisponibilidad(PDO $conexion, int $medico, string $fechaHora): void
{
    ...
    if (!$fecha) {
        responderError('La fecha y hora no tienen un formato valido.');
    }
    $dias = [
        1 => 'LUNES',
        2 => 'MARTES',
    ...
    if (!$s->fetchColumn()) {
        responderError('El medico no tiene disponibilidad en ese horario.', 409);
    }
}
...
function autorizarGestionCita(PDO $conexion, object $usuario, array $cita): void
{
    ...
    if ($rol === 2) {
        ...
        if ((int) $perfil->fetchColumn() !== (int) $cita['id_medico']) {
            responderError('Solo puede gestionar las citas asignadas a su perfil.', 403);
        }
    ...
    if ((int) $perfil->fetchColumn() !== (int) $cita['id_paciente']) {
        responderError('Solo puede gestionar sus propias citas.', 403);
    }
}
ejecutarApi(function () use ($conexion, $metodo): void {
    ...
    if ($metodo === 'GET') {
        ...
        if ((int) $usuario->id_rol === 3) {
            ...
            if (!$idMedico) {
                responderError('La cuenta no tiene un perfil médico asociado.', 403);
            }
    ...
    if ($metodo === 'POST') {
        ...
        if (!$idPaciente) {
            responderError('La cuenta no tiene un perfil de paciente asociado.', 403);
        }

        foreach ([['medicos', 'id_medico'], ['salas', 'id_sala']] as [$t, $c]) {
            if (!existe($conexion, $t, $c, (int) $d[$c])) {
                responderError("El {$c} indicado no existe.");
            }
        }
        ...
        $s->execute([
            ':fecha' => $fecha,
            ':medico' => (int) $d['id_medico'],
            ':sala' => (int) $d['id_sala']
        ]);

        if ($s->fetch()) {
            responderError('El medico o la sala ya tienen una cita en esa fecha y hora.', 409);
        }
        $s = $conexion->prepare(
            'INSERT INTO citas(id_paciente, id_medico, id_sala, fecha_hora, motivo_consulta, estado)
                VALUES (:paciente, :medico, :sala, :fecha, :motivo, "PENDIENTE")'
        );
        $s->execute([
            ':paciente' => $idPaciente,
            ':medico' => (int) $d['id_medico'],
            ':sala' => (int) $d['id_sala'],
            ':fecha' => $fecha,
            ':motivo' => $d['motivo_consulta'] ?? null
        ]);
        responder([
            'success' => true,
            'id_cita' => (int) $conexion->lastInsertId()
        ], 201);
    }
    ...
    if ($metodo === 'PUT') {
        ...
        if (isset($d['fecha_hora'])) {
            ...
            $conflicto->execute([
                ':id' => $idCita,
                ':fecha' => $fecha,
                ':medico' => (int) $cita['id_medico'],
                ':sala' => (int) $cita['id_sala']
            ]);

            if ($conflicto->fetchColumn()) {
                responderError('El médico o la sala ya tienen una cita en esa fecha y hora.', 409);
            }
            ...
            responder([
                'success' => true,
                'mensaje' => 'Cita reprogramada. Requiere confirmación médica.',
                'estado' => 'PENDIENTE',
                'fecha_hora' => $fecha
            ]);
        }
