...
ejecutarApi(function () use ($conexion, $metodo): void {
    ...
    if ($metodo === 'GET') {
        ...
        if (isset($_GET['id']) && !$data) {
            responderError('Administrador no encontrado.', 404);
        }
        responder([
            'success' => true,
    ...
    if (!existe($conexion, 'usuarios', 'id_usuario', (int) $d['id_usuario'])) {
    ...
    if ($metodo === 'POST') {
        $s = $conexion->prepare(
            'INSERT INTO administradores(id_usuario, nombre, apellido, telefono)
                VALUES (:usuario, :nombre, :apellido, :telefono)'
        );
        $s->execute($p);
        responder([
            'success' => true,
            'id_administrador' => (int) $conexion->lastInsertId()
        ], 201);
    }
