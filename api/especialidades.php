...
ejecutarApi(function () use ($conexion, $metodo): void {
    if ($metodo === 'GET') {
        $sql = 'SELECT
                ...
                nombre_especialidad,
                descripcion
            FROM especialidades'; $params = [];

        if (isset($_GET['id'])) {
            $sql .= ' WHERE id_especialidad = :id'; $params[':id'] = idRequerido();
        }
    ...
    if ($metodo === 'DELETE') {
        $stmt = $conexion->prepare('DELETE FROM especialidades WHERE id_especialidad = :id'); $stmt->execute([':id' => idRequerido()]);
