...
ejecutarApi(function () use ($conexion, $metodo): void {
    ...
    foreach ([['usuarios', 'id_usuario'], ['especialidades', 'id_especialidad']] as [$t, $c]) {
    ...
    $p = [
        ...
        ':especialidad' => (int) $d['id_especialidad'],
        ':nombre' => trim($d['nombre']),
        ':apellido' => trim($d['apellido']),
        ':colegiado' => $d['colegiado_num'] ?? null,
        ':telefono' => trim($d['telefono'])
    ];

    if ($metodo === 'POST') {
        $s = $conexion->prepare(
            'INSERT INTO medicos(id_usuario, id_especialidad, nombre, apellido, colegiado_num, telefono)
                VALUES (:usuario, :especialidad, :nombre, :apellido, :colegiado, :telefono)'
        );
        $s->execute($p);
        responder([
            'success' => true,
            'id_medico' => (int) $conexion->lastInsertId()
        ], 201);
    }
