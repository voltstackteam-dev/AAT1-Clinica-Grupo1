...
export class AdminDashboardComponent implements OnInit {
  ...
  vistaActiva = signal<'citas' | 'horarios'>('citas');
  ...
  cargarHorarios(): void {
    ...
    this.http.get<any>(`${this.api}/horarios.php?id_medico=${this.medicoActual()}`).subscribe({
      next: (respuesta) => this.horarios.set(respuesta.data || []),
      error: () => this.mensaje.set('No se pudieron cargar los horarios.'),
    });
  }
  cambiarVista(vista: 'citas' | 'horarios'): void {
    this.vistaActiva.set(vista);
    this.mensaje.set('');
    if (vista === 'horarios') this.cargarHorarios();
  }
  ...
  puedeConfirmar(cita: any): boolean {
    return cita.estado === 'PENDIENTE' && (this.esMedico || this.esAdministrador);
  }
  ...
  puedeReprogramar(cita: any): boolean {
    return (
      (this.esMedico || this.esAdministrador) && ['PENDIENTE', 'CONFIRMADA'].includes(cita.estado)
    );
  }
  actualizarEstado(cita: any, estado: string): void {
    if (estado === 'CANCELADA' && !confirm(`¿Desea rechazar o cancelar la cita #${cita.id_cita}?`))
      return;
    this.actualizandoCita.set(cita.id_cita);
    this.http.put<any>(`${this.api}/citas.php`, { id_cita: cita.id_cita, estado }).subscribe({
      next: (respuesta) => {
      ...
      error: (error) => {
        this.mensaje.set(error.error?.mensaje || 'No se pudo actualizar el estado de la cita.');
        this.actualizandoCita.set(null);
      },
    });
  }
  ...
  guardarHorario(): void {
    ...
    solicitud.subscribe({
      next: () => {
        this.mensaje.set(
          id ? 'Horario actualizado correctamente.' : 'Horario guardado correctamente.',
        );
        this.editandoHorario.set(null);
        this.cargarHorarios();
      },
      error: (error) => this.mensaje.set(error.error?.mensaje || 'No se pudo guardar el horario.'),
    });
  }
  ...
  eliminarHorario(horario: any): void {
    if (!confirm(`¿Desea eliminar el horario del ${horario.dia_semana}?`)) return;
    this.http.delete<any>(`${this.api}/horarios.php?id=${horario.id_disponibilidad}`).subscribe({
      next: () => {
        this.mensaje.set('Horario eliminado.');
        this.cargarHorarios();
      },
      error: (error) => this.mensaje.set(error.error?.mensaje || 'No se pudo eliminar el horario.'),
    });
  }
}
